...
from numpy import argmin, argsort, log, floor, sum, fmax, sqrt, exp, full, eye, diag, apply_along_axis, round, any, asarray, dot, random as rand
from numpy.linalg import norm, cholesky, eig, lstsq
...
def CovarianceMaatrixAdaptionEvolutionStrategyF(task, epsilon=1e-20, rnd=rand):
	...
	while not task.stopCondI():
		...
		ps = lstsq(cholesky(C) + epsilon, (1 - cs) * ps + sqrt(cs * (2 - cs) * mueff) * M + epsilon, rcond=None)[0]
		sigma *= exp(cs / ds * (norm(ps) / ENN - 1)) ** 0.3
		if norm(ps) / sqrt(1 - (1 - cs) ** (2 * (task.Iters + 1))) < hth: hs = 1
		else: hs = 0
		delta = (1 - hs) * cc * (2 - cc)
		pc = (1 - cc) * pc + hs * sqrt(cc * (2 - cc) * mueff) * M
		C = (1 - c1 - cmu) * C + c1 * (dot(pc.conj().T, pc) + delta * C)
		for i in range(mu): C += cmu * w[i] * dot(pop_step[i].conj().T, pop_step[i])
		E, V = eig(C)
		if any(E < 0):
			E = fmax(E, 0)
