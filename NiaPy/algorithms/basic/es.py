...
from numpy import argmin, argsort, log, floor, sum, fmax, sqrt, exp, full, eye, diag, apply_along_axis, round, any, asmatrix, real
...
class EvolutionStrategyMpL(EvolutionStrategy1p1):
	...
	**Reference paper:**
	...
	def setParameters(self, lam=45, **kwargs):
		r"""Set the arguments of an algorithm.
...
class CovarianceMaatrixAdaptionEvolutionStrategy(Algorithm):
	...
	**Reference paper:**
	...
	def runTask(self, task):
		lam, alpha_mu, hs = (4 + round(3 * log(task.D))) * 10, 2, 0
		mu = int(round(lam / 2))
		...
		ps, pc, C, sigma, M = full(task.D, 0.0), full(task.D, 0.0), eye(task.D), 0.3 * task.bRange, full(task.D, 0.0)
		x = self.uniform(task.Lower, task.Upper)
		x_f = task.eval(x)
		while not task.stopCondI():
			pop_step = [self.Rand.multivariate_normal(full(task.D, 0.0), C) for _ in range(int(lam))]
			pop = [task.repair(x + sigma * ps) for ps in pop_step]
			pop_f = apply_along_axis(task.eval, 1, pop)
			isort = argsort(pop_f)
			pop, pop_f = [pop[isort[i]] for i in range(mu)], [pop_f[isort[i]] for i in range(mu)]
			for i in range(mu): M += w[i] * pop_step[i]
			xn = x + sigma * M
			xn_f = task.eval(xn)
			if xn_f < x_f: x, x_f = xn, xn_f
			if pop_f[0] < x_f: x, x_f = pop[0], pop_f[0]
			ps = ((1 - cs) * ps + sqrt(cs * (2 - cs) * mueff) * M + self.epsilon) / (cholesky(C) + self.epsilon)
			sigma = sigma * exp(cs / ds * (norm(ps) / ENN - 1)) ** 0.3
			if norm(ps) / sqrt(1 - (1 - cs) ** (2 * (task.Iters + 1))) < hth: hs = 1
			else: hs = 0
			delta = (1 - hs) * cc * (2 - cc)
			pc = (1 - cc) * pc + hs * sqrt(cc * (2 - cc) * mueff) * M
			C = (1 - c1 - cmu) * C + c1 * (asmatrix(pc).H * pc + delta * C)
			for i in range(mu): C = C * cmu * w[i] * asmatrix(pop_step[i]).H * pop_step[i]
			E, V = eig(C)
			if any(E < 0):
				E = fmax(E, 0)
				C = real(V * diag(E) / V)
