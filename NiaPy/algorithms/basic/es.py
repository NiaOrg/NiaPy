...
class EvolutionStrategy1p1(Algorithm):
	...
	def runIteration(self, task, c, fpop, xb, fxb, ki, **dparams):
		...
		cn = to_object_array([task.repair(self.mutate(c.x, c.rho), self.Rand) for _i in range(self.mu)])
...
class EvolutionStrategyMpL(EvolutionStrategy1p1):
	...
	def runIteration(self, task, c, fpop, xb, fxb, ki, **dparams):
		...
		cn = to_object_array([IndividualES(x=self.mutateRand(c, task), task=task, rnd=self.Rand) for _ in range(self.lam)])
		cn = append(cn, c)
		cn = to_object_array([cn[i] for i in argsort([i.f for i in cn])[:self.mu]])
...
class EvolutionStrategyML(EvolutionStrategyMpL):
	...
	def newPop(self, pop):
		...
		if self.mu < self.lam: return to_object_array([pop[i] for i in pop_s[:self.mu]])
		npop = list()
	...
	def runIteration(self, task, c, fpop, xb, fxb, **dparams):
		...
		Returns:
			Tuple[numpy.ndarray, numpy.ndarray, numpy.ndarray, float, Dict[str, Any]]:
				...
				5. Additional arguments.
		"""
		cn = to_object_array([IndividualES(x=self.mutateRand(c, task), task=task, rand=self.Rand) for _ in range(self.lam)])
		c = self.newPop(cn)
