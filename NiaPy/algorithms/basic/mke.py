...
class MonkeyKingEvolutionV1(Algorithm):
	...
	def moveP(self, x, x_pb, x_b, task):
		...
		"""
		return x_pb + self.F * self.random(task.D) * (x_b - x)
	...
	def moveMK(self, x, task):
		...
		"""
		return x + self.FC * self.random([int(self.C * task.D), task.D]) * x
	...
	def moveMokeyKingPartice(self, p, task):
		...
		"""
		p.MonkeyKing = False
		A = np.apply_along_axis(task.repair, 1, self.moveMK(p.x, task), self.rng)
	...
	def initPopulation(self, task):
		...
		for i in self.rng.choice(self.NP, int(self.R * len(pop)), replace=False): pop[i].MonkeyKing = True
		return pop, fpop, {}
...
class MonkeyKingEvolutionV2(MonkeyKingEvolutionV1):
	...
	def moveMokeyKingPartice(self, p, pop, task):
		...
		for _i in range(int(self.C * self.NP)):
			r = self.rng.choice(self.NP, 2, replace=False)
			a = task.repair(self.moveMK(p.x, pop[r[0]].x - pop[r[1]].x, task), self.rng)
			a_f = task.eval(a)
...
class MonkeyKingEvolutionV3(MonkeyKingEvolutionV1):
	...
	def runIteration(self, task, X, X_f, xb, fxb, k, c, **dparams):
		...
		Returns:
			Tuple[numpy.ndarray, numpy.ndarray[float], Dict[str, Any]]:
				...
				3. Additional arguments:
					...
					* c (int): Constant.
		"""
		X_gb = np.apply_along_axis(task.repair, 1, xb + self.FC * X[self.rng.choice(len(X), c)] - X[self.rng.choice(len(X), c)], self.rng)
		...
		for i in range(self.NP): self.rng.shuffle(M[i])
		X = np.apply_along_axis(task.repair, 1, M * X + np.vectorize(self.neg)(M) * xb, self.rng)
