...
class FishSchoolSearch(Algorithm):
    ...
    def update_steps(self, task):
        ...
        Returns:
            Tuple[numpy.ndarray, numpy.ndarray]: TODO.
        """
        curr_step_individual = np.full(task.D, self.step_individual_init - task.Iters * np.float(self.step_individual_init - self.step_individual_final) / task.nGEN)
        curr_step_volitive = np.full(task.D, self.step_volitive_init - task.Iters * np.float(self.step_volitive_init - self.step_volitive_final) / task.nGEN)
