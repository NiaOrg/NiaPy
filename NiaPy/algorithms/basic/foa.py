...
import logging
from numpy import random as rand, where, mgrid, apply_along_axis, zeros, append, tile, ndarray, array, repeat, delete, argsort, arange, bool as npbool
...
class ForestOptimizationAlgorithm(Algorithm):
	...
	**Reference paper:** Manizheh Ghaemi, Mohammad-Reza Feizi-Derakhshi, Forest Optimization Algorithm, Expert Systems with Applications, Volume 41, Issue 15, 2014, Pages 6676-6687, ISSN 0957-4174, https://doi.org/10.1016/j.eswa.2014.05.009.
	...
	def localSeeding(self, task, trees):
		...
		for i in range(0,n):
			for j in range(0, self.lsc):
				...
				delta = rand.uniform(-task.benchmark.Upper, task.benchmark.Upper)	#should this be: (task.benchmark.Lower, task.benchmark.Upper)?
				newTree[di] += delta
				newTrees[i + j] = newTree
	...
	def survivalOfTheFittest(self, task, trees, candidates):
		evaluated = apply_along_axis(task.eval, 1, trees[:,1:])	#evaluate population
		ei = evaluated.argsort()	#get sorted indices
		...
		evaluated = evaluated[ei[self.al:]]
		return trees, candidates, evaluated
	...
	def runTask(self, task):
		...
		while not task.stopCondI():
			...
			forest, candidatePopulation, evaluated = self.survivalOfTheFittest(task, forest, candidatePopulation)
			...
			if gsn > 0:
				...
				evaluated = append(evaluated, gste)
				ei = evaluated.argsort()	#get sorted indices
				forest = forest[ei]	#sort according to the indices
			
			#update the best tree so far
			forest[0, 0] = 0
		
		evaluated = evaluated[ei]
		return forest[0], evaluated[0]
...
algorithm = ForestOptimizationAlgorithm(nFES=100000, NP=10, D=9, lt=3, al=10, tr=0.2, benchmark=MyBenchmark())
best = algorithm.run()
print(best)