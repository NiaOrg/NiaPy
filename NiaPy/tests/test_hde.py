...
class MSDEMTSTestCase(AlgorithmTestCase):
	def test_custom_works_fine(self):
		ca_custom = MultiStratgyDifferentialEvolutionMTS(NP=40, seed=self.seed)
		ca_customc = MultiStratgyDifferentialEvolutionMTS(NP=40, seed=self.seed)
		AlgorithmTestCase.algorithm_run_test(self, ca_custom, ca_customc, MyBenchmark())

	def test_griewank_works_fine(self):
		ca_griewank = MultiStratgyDifferentialEvolutionMTS(NP=40, seed=self.seed)
		ca_griewankc = MultiStratgyDifferentialEvolutionMTS(NP=40, seed=self.seed)
		AlgorithmTestCase.algorithm_run_test(self, ca_griewank, ca_griewankc)
