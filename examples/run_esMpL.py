...
from NiaPy.util import StopingTask, TaskConvPrint, TaskConvPlot, OptimizationType, getDictArgs
...
def simple_example(alg, runs=10, D=10, nFES=50000, nGEN=10000, seed=[None], optType=OptimizationType.MINIMIZATION, optFunc=MinMB, **kn):
	for i in range(runs):
		task = StopingTask(D=D, nFES=nFES, nGEN=nGEN, optType=optType, benchmark=optFunc())
...
if __name__ == '__main__':
	pargs, algo = getDictArgs(sys.argv[1:]), EvolutionStrategyMpL
