...
def simple_example(runs=10):
	for i in range(runs):
		algo = EvolutionStrategyML(D=50, nFES=50000, seed=None, benchmark=MyBenchmark())
		best = algo.run()
		logger.info('%s %s' % (best[0], best[1]))

def logging_example():
	task = TaskConvPrint(D=10, nFES=50000, nGEN=50000, benchmark=MyBenchmark())
	algo = EvolutionStrategyML(mu=50, lam=40, k=60, c_a=1.5, c_r=0.25, seed=None, task=task)
	best = algo.run()
...
def plot_example():
	...
	algo = EvolutionStrategyML(mu=65, lam=50, k=25, c_a=1.5, c_r=0.25, seed=None, task=task)
	best = algo.run()
	logger.info('%s %s' % (best[0], best[1]))
