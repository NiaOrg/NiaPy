...
def simple_example(runs=10, D=10, nFES=50000):
	for i in range(runs):
		algo = GlowwormSwarmOptimizationV1(D=D, nFES=nFES, n=50, nt=5, l0=5, rho=0.4, gamma=0.6, beta=0.08, s=0.3, seed=None, benchmark=MyBenchmark())
		best = algo.run()
		logger.info('%s %s' % (best[0], best[1]))

def logging_example(D=10, nFES=50000):
	task = TaskConvPrint(D=D, nFES=nFES, nGEN=10000, benchmark=MyBenchmark())
	algo = GlowwormSwarmOptimizationV1(n=25, nt=5, l0=5, rho=0.2, gamma=0.3, beta=0.05, s=0.05, seed=None, task=task)
	best = algo.run()
	logger.info('%s %s' % (best[0], best[1]))

def plot_example(D=10, nFES=50000):
	task = TaskConvPlot(D=D, nFES=nFES, nGEN=10000, benchmark=MyBenchmark())
	algo = GlowwormSwarmOptimizationV1(n=50, nt=5, l0=5, rho=0.4, gamma=0.6, beta=0.08, s=0.03, seed=None, task=task)
	best = algo.run()
	logger.info('%s %s' % (best[0], best[1]))
