...
def simple_example(runs=10):
	for i in range(runs):
		algo = GlowwormSwarmOptimization(D=50, nFES=1000, n=50, nt=5, l0=5, rho=0.4, gamma=0.6, beta=0.08, s=0.3, benchmark=MyBenchmark())
		best = algo.run()
...
def plot_example():
	...
	algo = GlowwormSwarmOptimization(n=50, nt=5, l0=5, rho=0.4, gamma=0.6, beta=0.08, s=0.03, task=task)
	best = algo.run()
	logger.info('%s %s' % (best[0], best[1]))
